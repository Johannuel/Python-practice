...
5. **Habilidades de Soporte Ultra-Fundamentales (Los Superpoderes)**
...
#### 🎮 Ruta de Aprendizaje Práctico Godot 2D/3D (Paso a Paso):
1. **Instalación e Interfaz:** Configuración del editor, comprensión del espacio de trabajo 2D, 3D, Script y el gestor de assets (AssetLib).
2. **Concepto de Nodos y Escenas:** Entender que todo en Godot es un Nodo. Composición de escenas complejas y modulares mediante instanciación de subescenas.
3. **GDScript Práctico:** Estructura de un script, tipado estático, variables y funciones fundamentales del ciclo de vida del motor (`_init()`, `_ready()`, `_process(delta)`, `_physics_process(delta)`).
4. **Entrada de Usuario y Movimiento:** Configurar acciones de entrada en Project Settings. Crear un script para mover un nodo (ej. `CharacterBody2D` o un Sprite2D) controlando velocidad, fricción y aceleración.
5. **Física y Colisiones de Objetos:** Diferencia práctica entre `CharacterBody2D` (movimiento manual con física), `StaticBody2D` (paredes y suelo inamovibles), `RigidBody2D` (física controlada por el motor) y `Area2D` (detección de triggers sin colisiones físicas para monedas o zonas de muerte).
6. **El Sistema de Señales (Signals):** Comunicación limpia y desacoplada (Observer pattern). Conectar señales usando la interfaz visual y de manera dinámica a través de código (`connect()` o `signal_name.connect()`).
7. **Interfaz de Usuario (UI):** Uso de nodos de control (`Control`) y contenedores (`VBoxContainer`, `HBoxContainer`, `GridContainer`, `MarginContainer`) para crear menús fluidos, barras de vida y overlays que se adapten a cualquier resolución.
8. **Proyecto Práctico Inicial:** Construir desde cero un juego clásico completo (un clon de Pong, Flappy Bird, o Space Invaders) implementando todos los conceptos aprendidos, organizando los archivos de manera estructurada y documentando el código.
